...
def get_pages(request, cur_course_user, submission_id, assessment_id=None):
  submission_pages = models.SubmissionPage.objects.filter(submission=submission_id).order_by('page_number')
  pages = [page.page_number for page in submission_pages]
  return http.HttpResponse(json.dumps(pages), mimetype='application/json')
